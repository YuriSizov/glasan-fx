...
class_name OPNVoice extends Voice
...
func _init(op_count: int = 1) -> void:
	...
	voice.set_chip_type(SiONDriver.CHIP_OPN)
	_channel_param_count = OPN_CH_PARAMS
	_operator_param_count = OPN_OP_PARAMS

	data.clear()
	data.resize(OPN_CH_PARAMS + OPN_OP_PARAMS * op_count)
...
func _add_operator() -> void:
	...
	var data_index := OPN_CH_PARAMS + OPN_OP_PARAMS * operator_index
	...
	data[data_index + 9] = VoiceKnob.new("AM", 0, 3)
