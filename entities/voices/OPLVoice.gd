...
class_name OPLVoice extends Voice
...
func _init(op_count: int = 1) -> void:
	...
	voice.set_chip_type(SiONDriver.CHIP_OPL)
	_channel_param_count = OPL_CH_PARAMS
	_operator_param_count = OPL_OP_PARAMS

	data.clear()
	data.resize(OPL_CH_PARAMS + OPL_OP_PARAMS * op_count)
...
func _add_operator() -> void:
	...
	var data_index := OPL_CH_PARAMS + OPL_OP_PARAMS * operator_index
	...
	data[data_index + 4]  = VoiceKnob.new("EG", 0, 1)
