...
# Implementation taken from Bosca Ceoil Blue.
...
static func write_uint32(array: PackedByteArray, value: int, big_endian: bool = false) -> void:
	if big_endian: # PackedByteArray encodes in Little Endian with no setting to change that.
		var temp := PackedByteArray()
		temp.resize(4)
		temp.encode_u32(0, value)
		temp.reverse() # So we encode it using a temp array which we then reverse.
...
static func write_uint16(array: PackedByteArray, value: int, big_endian: bool = false) -> void:
	if big_endian: # PackedByteArray encodes in Little Endian with no setting to change that.
		var temp := PackedByteArray()
		temp.resize(2)
		temp.encode_u16(0, value)
		temp.reverse() # So we encode it using a temp array which we then reverse.
	...
	var offset := array.size()
	array.resize(offset + 2)
	array.encode_u16(offset, value)


static func write_int8(array: PackedByteArray, value: int) -> void:
	var offset := array.size()
	array.resize(offset + 1)
	array.encode_s8(offset, value)


static func write_uint8(array: PackedByteArray, value: int) -> void:
	var offset := array.size()
	array.resize(offset + 1)
