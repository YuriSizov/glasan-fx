...
func _draw() -> void:
	...
	for i in range(1, VU_COUNT + 1):
		...
		var bar_position := Vector2(
			...
			offset.y + _render_size.y - height
		)
...
func _get_height(index: int, magnitude: float) -> float:
	var base_value: float = clamp((MIN_DB + linear_to_db(magnitude)) / MIN_DB, 0, 1) * _render_size.y
